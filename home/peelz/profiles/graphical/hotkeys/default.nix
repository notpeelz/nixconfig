...
with lib;
{
  ...
  config.my.graphical.services.sxhkd = mkIf config.my.graphical.enable {
    ...
    # Unfortunately, there is currently no way to override env vars
    # from EnvironmentFile= using Environment=
    # We have to reference full paths instead
    # See https://github.com/systemd/systemd/issues/9788
    hotkeys = let
    ...
    in (optionals config.my.graphical.wm.bspwm.enable [
      ...
      {
        hotkey = "hyper + Return";
        cmd = "${config.pam.sessionVariables.TERMINAL}";
      }

      # Open browser
      {
        hotkey = "hyper + {_,shift + }n";
        cmd = "${config.pam.sessionVariables.BROWSER} {_, ${browserIncognitoParam}}";
      }
